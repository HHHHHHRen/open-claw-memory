...
%% 关键修改：扩展计算域，让边界节点完整显示
% 左右各扩展一个节点半径，确保边界节点完整
num_cross = 3;
Lx_core = num_cross * diamond_diag;  % 核心区域（3个cross）
Ly_core = diamond_diag;

% 扩展边界，让节点完整
Lx = Lx_core + 2 * r_node;  % 左右各扩展 r_node
Ly = Ly_core + 2 * r_node;  % 上下各扩展 r_node
...
% 下层纤维（+45°）
for c = -(Ly+r_node):diamond_diag:(Lx+Ly+r_node)  % 扩展范围
...
% 上层纤维（-45°）
for c = -r_node:diamond_diag:(Lx+Ly+r_node)  % 扩展范围
    dist_xy = abs(X(:,:,1) + Y(:,:,1) - c) / sqrt(2);
...
%% 可视化
figure('Name', '完整节点隔网', 'Position', [30 30 1600 450]);

[ys, xs, zs] = ind2sub([NY, NX, NZ], find(solid));

% XY俯视图（显示扩展区域，但标记核心3个cross）
subplot(1,4,1);
...
plot([0 NX], [1.0/dx 1.0/dx], 'g--', 'LineWidth', 1);
plot([0 NX], [1.22/dx 1.22/dx], 'r--', 'LineWidth', 1);
hold off;
