...
diamond_diag = pitch * sqrt(2);  % ≈ 5.657 mm（单个菱形对角线）

%% 计算域：3×3个cross
num_cross = 3;  % X和Y方向都是3个
Lx_core = num_cross * diamond_diag;  % X方向3个cross
Ly_core = num_cross * diamond_diag;  % Y方向3个cross（关键修改）
...
fprintf('=== 3×3阵列隔网 ===\n');
fprintf('单个菱形对角线: %.3f mm\n', diamond_diag);
fprintf('核心区域: %.2f×%.2f mm (3×3个cross)\n', Lx_core, Ly_core);
fprintf('网格: %d×%d×%d = %.2f万\n', NX, NY, NZ, NX*NY*NZ/10000);
...
%% 可视化
figure('Name', '3×3阵列隔网', 'Position', [30 30 1600 450]);
...
axis equal tight;
xlabel('X'); ylabel('Y');
title(sprintf('XY俯视 3×3阵列 (%.1f%%)', porosity*100));
grid on;
hold on;
% 核心区域边界
...
for i = 1:2
    ...
    yv = y_core_start + i * round(diamond_diag/dx);
    plot([x_core_start x_core_end], [yv yv], 'r--', 'LineWidth', 1);
end
hold off;
